...
import { Search, TrendingUp, Layers, ArrowDown, Award, Coffee, AlertCircle, Box } from "lucide-react";
const capabilities = [{
  icon: Search,
  ...
  return <section className="py-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {capabilities.map((capability, index) => <motion.div key={capability.title} initial={{
          opacity: 0,
          y: 40
        }} whileInView={{
          opacity: 1,
          ...
          y: -8
              ...
              <div className="h-full p-8 bg-background border border-border hover:border-primary transition-all duration-300">
                ...
                <div className="space-y-2 mt-6 pt-6 border-t border-border">
                  {capability.metrics.map((metric, i) => <motion.div key={i} initial={{
                opacity: 0,
                x: -10
              }} whileInView={{
                opacity: 1,
                ...
                {/* Hover effect line */}
                <motion.div className="absolute bottom-0 left-0 h-[2px] bg-primary" initial={{
              scaleX: 0
            }} whileInView={{
              scaleX: 1
            }} viewport={{
